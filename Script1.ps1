...
$vms | Select-Object uuid,NodeUUID,name,Description,operatingSystem,state,@{Name="RAM Assigned";Expression={$_.mem /1GB}},numVCPU,uiState,machineType,cpuType,snapshotAllocationBlocks,lastSeenRunningOnNodeUUID,@{Name="Preferred Node";Expression={$_.affinityStrategy.preferredNodeUUID}},@{Name="Backup Node";Expression={$_.affinityStrategy.backupNodeUUID}} | export-csv -Path c:\temp\ScaleVMs.csv -Delimiter ";" -NoTypeInformation
